...
void __debug__print_uuid(struct bt_uuid_128 *uuid) 
{
    for (int i = 0; i < 16; i++)
        printk("%x", uuid->val[i]);
    printk("\n");
...
int check_id(struct bt_uuid_128 *uuid) 
{
...
int add_uuid(struct bt_uuid_128 *uuid) 
{
    ...
    uuids_count_buffer++;
    err = nvs_write(&nvs_file_system, uuids_count_buffer, uuid, sizeof(struct bt_uuid_128));
    if (err < 0)
    {
...
int get_uuid(uint8_t index, struct bt_uuid_128 *uuid) 
{
    ...
    err = nvs_read(&nvs_file_system, index+1, uuid, sizeof(struct bt_uuid_128));
    if (err < 0)
    {
